...
from slots.utils.date_time import convert_minutes_in_human_readable_format, get_timestamp, get_weekday_num, \
    time_difference
from slots.utils.view_helpers import generate_random_id, get_locale

from Accounts.models import User
from Accounts.constants import *
from Dashboard.models.clinics import ClinicReg
...
class Service(models.Model):
    ...
    currency = models.CharField(max_length=3, default='KES', validators=[MaxLengthValidator(3), MinLengthValidator(3)])
...
class StaffMember(models.Model):
    user = models.ForeignKey(User,
          ...
          limit_choices_to=Q(registered=True) | Q(open_for_mboka=True))
    ...
    clinic = models.ForeignKey(ClinicReg, on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"{self.get_staff_member_name()} - {self.role} at {self.clinic.clinic_name}"
...
class AppointmentRequest(models.Model):
    ...
    status = models.CharField('Status', max_length=10, default='Pending', choices=APPOINTMENT_STATUS_CHOICES, blank=False, )
    clinic = models.ForeignKey(ClinicReg, blank=True, null=True, on_delete=models.CASCADE, related_name='clinic_doc')
    ...
    additional_msg = RichTextField(blank=True)
    ...
    def __str__(self):
        return f"{self.appointment_number} - {self.date} - {self.start_time} to {self.end_time} - {self.service.name}"
    ...
    def get_start_time(self):
        return self.time_of_appointment

    def get_service_duration(self):
        return Service.get_duration()
...
class RescheduleHistory(models.Model):
    ...
    class Meta:
        verbose_name = _("Reschedule History")
        verbose_name_plural = _("Reschedule Histories")
        ordering = ['-created_at']
...
class Appointment(models.Model):
    client  = models.ForeignKey(User, on_delete=models.SET_NULL, related_name='patients', null=True, blank=False)
    ...
    id_request = models.CharField(max_length=100, blank=True, null=True)
    remark = RichTextField(blank=True)
    prescription = RichTextField(blank=True)
    recommended_test = RichTextField(blank=True)
