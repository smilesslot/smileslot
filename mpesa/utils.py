...
from mpesa.exceptions import MpesaConfigurationException, IllegalPhoneNumberException, MpesaConnectionError, MpesaError
...
def mpesa_response(r):
	...
	r. app_key = json_response.get('app_key ', '')
	r. QRCode_encode = json_response.get('QRCode', '')
	...
	r.status_Message_Invoice = json_response.get('Status_Message')
...
def api_base_url():
	...
	if mpesa_environment == 'development':
		...
		raise MpesaConfigurationException('Mpesa environment not configured properly - MPESA_ENVIRONMENT should be sandbox or production.cer')
...
def B_generate_access_token_request(consumer_key=None, consumer_secret=None):
	...
	consumer_key = consumer_key if consumer_key is not None else mpesa_config('MPESA_CONSUMER_KEY_B') 
	consumer_secret = consumer_secret if consumer_secret is not None else mpesa_config('MPESA_CONSUMER_SECRET_B')
...
def B_generate_access_token():
	...
	r =B_generate_access_token_request()
	if r.status_code != 200:
		# Retry to generate access token
		r = B_generate_access_token_request()
		if r.status_code != 200:
			raise MpesaError('Unable to generate Business PayBill access token')

	token = r.json()['access_token']

	AccessToken.objects.all().delete()
	B_access_token = AccessToken.objects.create(token=token)

	return B_access_token

def B_mpesa_access_token():
	"""
