...
class AdminDashboardView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["total_revenue"] = (
            Prescription.objects.filter(status="completed").aggregate(
        ...
        for doctor in doctors:
            doctor.earned = (
                Prescription.objects.filter(doctor=doctor, status="completed").aggregate(
                    total=Sum("doctor__profile__price_per_consultation")
                )["total"]
                or 0
            )
        ...
        for patient in patients:
            ...
            patient.total_paid = (
                Prescription.objects.filter(patient=patient, status="completed").aggregate(
                    total=Sum("doctor__profile__price_per_consultation")
                )["total"]
                or 0
            )
