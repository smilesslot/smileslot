...
run_tests() {
  ...
  for py_ver in "${python_versions[@]}"; do
    (
      ...
      if docker build -f Dockerfile.test --build-arg PYTHON_VERSION="${py_ver}" --build-arg DJANGO_VERSION="${dj_ver}" -t django_smileslot_test:"${py_ver}_${dj_ver}" . && \
         docker run --rm django_smileslot_test:"${py_ver}_${dj_ver}"; then
